...
class BaseNestedRouter(
    ...
    def _get_parent(self, request, parent_id: int):
    ...
    def _register_routes(self):
        ...
        def create_item(request: Any, parent_id: int, payload: self.create_schema):  # type: ignore
            parent = self._get_parent(request, parent_id)
            service = self._get_service(request)
            service.create(payload, parent)
        ...
        def list_items(request: Any, parent_id: int):
            parent = self._get_parent(request, parent_id)
            service = self._get_service(request)
            return resp(HTTPStatus.OK, service.list(parent))
        ...
        def update_item(request: Any, parent_id: int, id: int, payload: self.update_schema):  # type: ignore
            service = self._get_service(request)
